...
var Spawner = {
    ...
    spawnMiner: function(spawn, harvestTarget) {
        return Spawner.spawnIdealWorker(
            spawn,
    ...
    spawnWorker: function(spawn) {
        ...
        const parts = [];
        const maxParts = Math.min(Math.floor(capacity / 200), WORKER_MAX_WORK);
        for (var ii = 0; ii < maxParts; ii++) {
            parts.push(WORK, CARRY, MOVE);
        }
        return spawn.createCreep(parts, undefined, {role: 'worker'});
    },
    ...
    spawnMule: function(spawn, assignedRoom) {
        ...
        const parts = [];
        for (var ii = 0; ii < MULE_MAX_CARRY / 2; ii++, capacity -= 150) {
            parts.push(CARRY, CARRY, MOVE);
        }

        return spawn.createCreep(parts, undefined, {role: 'mule', assignedRoom});
    },
    ...
    run: function(spawn) {
        ...
        if (!counts['worker'] || counts['worker'] < 1) {
            Spawner.spawnIdealWorker(spawn, [WORK, CARRY, MOVE], {role: 'worker'});
        } else {
            var plan = ExpansionPlanner.getRoomDevelopmentPlan(spawn.room);
            if (plan.action == 'spawn_worker' && counts['worker'] < 10) {
                Spawner.spawnWorker(spawn);
            } else if (plan.action == 'spawn_miner') {
